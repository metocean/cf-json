    ...
    def to_dict(self, mapping={}):
        ...
        for var in self.variables:
            ...
            try:
                ...
                if var == "time":
                    ...
                    continue
                vardims = [
                    d for d in self.variables[var].dimensions if d in res["dimensions"]
                ]
                ...
                for att in self.variables[var].ncattrs():
                    if att not in ["missing_value", "cell_methods"]:
                        res["variables"][varout]["attributes"].update(
                            {str(att): print_val(self.variables[var].getncattr(att))}
                        )
            except:
        ...
        for var in self.variables:
            ...
            try:
                ...
                rawvals = (
                    numpy.ma.array(self.variables[var][:]).filled(numpy.nan).squeeze()
                )
                if var == "time" and "units" in self.variables[var].ncattrs():
                    if "calendar" in self.variables[var].ncattrs():
                        times = num2date(
                            rawvals,
                            self.variables[var].getncattr("units"),
                            self.variables[var].getncattr("calendar"),
                        )
                    else:
                        times = num2date(
                            rawvals, self.variables[var].getncattr("units")
                        )
    ...
    def from_json(self, js, mapping={}):
        ...
        for var in dico["variables"]:
            ...
            for att in dico["variables"][var]["attributes"]:
                try:
                    if att != "_FillValue":
                        self.variables[var].setncattr(
                            att, dico["variables"][var]["attributes"][att]
                        )
                except:
                    print(
                        "Failed to set variable %s attribute %s to %s"
                        % (var, att, str(dico["variables"][var]["attributes"][att]))
                    )

            try:
                self.variables[var][:] = numpy.array(dico["data"][var]).reshape(
                    self.variables[var].shape
                )
            except:
