...
class CFJSONinterface(object):
    ...
    def to_dict(self, mapping):
        ...
        for var in self._obj.variables:
            try:
                ...
                if var == "time":
                    ...
                    continue
                vardims = [
                    d for d in self._obj.variables[var].dims if d in res["dimensions"]
                ]
        ...
        for var in self._obj.variables:
            ...
            try:
                ...
                if var == "time":
                    vals = [
                        t.strftime("%Y-%m-%dT%H:%M:%SZ") for t in to_datetime(rawvals)
                    ]
    ...
    def from_json(self, js):
        ...
        if "attributes" in dico.keys():
            ...
            logging.debug(
                "copying global attributes: {}".format(dico["attributes"].items())
            )
        ...
        for varname, var in six.iteritems(dico["variables"]):
            ...
            if "units" in var["attributes"] and "ISO8601" in var["attributes"]["units"]:
                logging.debug(
                    'found "ISO8601" in units string, guessing time variable, converting to datetime64'
                )
                ...
                if any([t.utcoffset() is not None for t in time_dt]):
                    if all([t.utcoffset() == dt.timedelta(0) for t in time_dt]):
                        ...
                        logging.warning(
                            "Mixed timezones (or mixed naive / aware) in input, may lead to dtype=object in output"
                        )
                ...
                if var["shape"]:
                    ...
                    if isinstance(var["data"], list):
                        if len(var["data"]) > 1:
                            msg = "len(data) > 1 not allowed with empty / missing shape; varname: {}".format(
                                varname
                            )
                            raise Exception(msg)
                ...
                if self._obj[varname].dtype == "O":
                    dtype_set = set(
                        [type(el) for el in self._obj[varname].data.flatten()]
                    )
                    if str not in dtype_set:
                        if float not in dtype_set:
                            logging.warning(
                                'casting variable "{}" to float to preserve None / NaN, but no floats in original data'.format(
                                    varname
                                )
                            )
